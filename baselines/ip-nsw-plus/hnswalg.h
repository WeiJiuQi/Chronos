...
#include <iostream>
#include <random>
#include <unordered_set>  // Constant-time candidate membership checks.
...
namespace hnswlib {
  ...
  public:
    ...
    HierarchicalNSW(SpaceInterface<dist_t> *s, size_t maxElements, size_t M, size_t cos_M, size_t efConstruction, size_t cos_efConstruction) :
      ...
      M_ = M;
      maxM_ = M_;  // Upper layers do not maintain separate inner-product graphs.
      maxM0_ = M_ * 2;
      efConstruction_ = efConstruction;
      ef_ = 7;  // Default query beam width; callers may override it with setEf().
    ...
    // GraphMIPS inherited a single shared random engine from the legacy
    // hnswlib implementation.  addPoint() may be called concurrently, so the
    // engine must be serialized just as it is in current hnswlib.  This mutex
    // is runtime-only and therefore does not change the serialized index
    // format.
    ...
    inline char *getExternalLabelBytes(tableint internal_id) {
    ...
    std::priority_queue< std::pair< dist_t, tableint  >> searchBaseLayer(tableint ep, void *datapoint, int layer) {
      ...
      while (!candidateSet.empty()) {
        ...
        if (size > 0) {
          _mm_prefetch((char *)(massVisited + datal[0]), _MM_HINT_T0);
          _mm_prefetch((char *)(massVisited + datal[0] + 64), _MM_HINT_T0);
    ...
    */
    std::priority_queue<std::pair<dist_t, tableint>> searchBaseLayerST(tableint ep, void *datapoint, size_t ef) {
      ...
      while (!candidateSet.empty()) {
        ...
        int size = *data;
        if (size > 0) {
          const int first = data[1];
          _mm_prefetch((char *)(massVisited + first), _MM_HINT_T0);
          _mm_prefetch((char *)(massVisited + first + 64), _MM_HINT_T0);
          _mm_prefetch(data_level0_memory_ + first * size_data_per_element_ + offsetData_, _MM_HINT_T0);
        }
        ...
        for (int j = 1; j <= size; j++) {
          int tnum = *(data + j);
          if (j < size) {
            const int next = data[j + 1];
            _mm_prefetch((char *)(massVisited + next), _MM_HINT_T0);
            _mm_prefetch(data_level0_memory_ + next * size_data_per_element_ + offsetData_, _MM_HINT_T0);////////////
          }
          if (!(massVisited[tnum] == currentV)) {
    ...
    template <bool is_construction, bool collect_metrics>
    std::priority_queue<std::pair<dist_t, tableint>> searchBaseLayerST_inner_product(
      ...
      while (!precandidateSet.empty()) {
        ...
        tableint curNodeNum = curr_el_pair.second;
        // During parallel construction another insertion may update this IP
        // adjacency list.  Query-only search runs after the index is frozen
        // and deliberately remains lock-free to preserve upstream latency.
        unique_lock<mutex> lock;
        if (is_construction) {
          lock = unique_lock<mutex>(ll_locks[curNodeNum]);
        }
      ...
      while (!candidateSet.empty()) {
        ...
        tableint curNodeNum = curr_el_pair.second;
        // See the construction/query distinction above.  A separate lock is
        // required for the second expansion phase because the first lock has
        // already left scope with its loop iteration.
        unique_lock<mutex> lock;
        if (is_construction) {
          lock = unique_lock<mutex>(ll_locks[curNodeNum]);
        }
    ...
    void getNeighborsByHeuristic2(tableint cur_c, std::priority_queue< std::pair< dist_t, tableint>> &topResults, const int NN)
    {
      ...
      while (resultSet.size()) {
        ...
        for (std::pair< dist_t, tableint> curen2 : returnlist) {
          ...
          // The coefficient can be adjusted to trade search selectivity for recall.
    ...
    void mips_mutuallyConnectNewElement(void *datapoint, tableint cur_c, std::priority_queue< std::pair< dist_t, tableint  >> topResults) {
      ...
      for (int idx = 0; idx < rez.size(); idx++) {
        ...
        // Lock the reciprocal adjacency list before adding the reverse edge.
        unique_lock<mutex> lock(ll_locks[rez[idx]]);
    ...
    void addPoint(void *datapoint, labeltype label, int level = -1) {
      ...
      int curlevel;
      {
        lock_guard<mutex> random_lock(level_generator_lock_);
      ...
      // Snapshot both fields while holding the same lock.  Reading the entry
      // point after unlocking races with a concurrent insertion that raises
      // the maximum level and publishes a new entry point.
      ...
      if (currObj != -1) {
        ...
        if (curlevel < maxlevelcopy) {
          ...
          // Greedy descent follows the single-candidate norm convention used
          // by the upstream GraphMIPS implementation.
        ...
        for (int level = min(curlevel, maxlevelcopy); level >= 0; level--) {
          ...
          // The entry point may change while descending through the levels.
          ...
          if (level == 0) {
            ...
            std::priority_queue<std::pair<dist_t, tableint>> mips_topResults =
              searchBaseLayerST_inner_product<true, false>(cos_res, datapoint, efConstruction_);
            // The MIPS connection routine performs its own neighbor selection.
    ...
    std::priority_queue< std::pair< dist_t, labeltype >> searchKnn(void *query_data, int k) {
      ...
      // Seed inner-product search with candidates from the angular graph.
      ...
      std::priority_queue<std::pair<dist_t, tableint>> mips_topResults =
        searchBaseLayerST_inner_product<false, true>(cos_res, query_data, ef_);
